...
if (isset($_POST['changepass'])) {
  if (isset($_POST['old_password']) && isset($_POST['new_password']) && isset ($_POST['confirm_password']) && isset ($_POST['username'])){
    ...
    if (empty($old_password)) {
      header("Location: ../profile.php?error=Parola veche este obligatorie");
      exit();
    } else if (empty($new_password)) {
      header("Location: ../profile.php?error=Parola nouă este obligatorie");
      exit();
    } else if (empty($confirm_password)) {
      header("Location: ../profile.php?error=Confirmarea parolei este obligatorie");
      exit();
    } else if ($new_password !== $confirm_password) {
      header("Location: ../profile.php?error=Parolele nu se potrivesc");
      exit();
    } else {
      $old_password = md5($old_password);
      $sql = "SELECT * FROM users WHERE password='$old_password' AND username='$username'";
      $result = mysqli_query($conn, $sql);
      if (mysqli_num_rows($result) === 1) {
        $new_password = md5($new_password);
        $sql2 = "UPDATE users SET password='$new_password' WHERE password='$old_password' AND username='$username'";
        $result2 = mysqli_query($conn, $sql2);
        if ($result2) {
          header("Location: ../profile.php?success=Parola a fost schimbată cu succes");
          exit();
        } else {
          header("Location: ../profile.php?error=Parola nu a putut fi schimbată");
          exit();
        }
      } else {
        echo $sql;
